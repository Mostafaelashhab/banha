@extends('layouts.app', ['title' => 'إدارة ' . \App\Models\Business::menuLabels($business->category)['title'] . ' · ' . $business->name])

@section('content')
@php
    $L         = \App\Models\Business::menuLabels($business->category);
...
<div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
                <h3 class="text-sm font-extrabold text-ink-950 inline-flex items-center gap-1.5 mb-0.5">
                    ...
                    QR · يفتح صفحة عامة لكل اللي تحت
                </h3>
                <p class="text-[11px] text-ink-500 leading-relaxed">طبعه أو شاركه مع زبائنك. أي حد بيصوّر الـQR يفتحله الصفحة على طول.</p>
                <div class="flex flex-wrap gap-1.5 mt-2">
                    ...
                    <button type="button" data-share data-share-url="{{ $menuUrl }}"
                            data-share-title="{{ $L['title'] }} · {{ $business->name }}"
        ...
        <div class="card-light p-4 mb-3" data-category="{{ $cat->id }}">
            <div class="flex items-center justify-between mb-3 pb-3 border-b border-ink-950/8">
                ...
                <form method="POST" action="{{ route('menu.category.destroy', $cat) }}"
                      data-confirm="حذف {{ $cat->name }}؟"
                      data-confirm-body="الأصناف اللي فيه مش هتتمسح، هتفقد القسم بس."
                      data-confirm-tone="danger">
                    @csrf @method('DELETE')
                    <button class="w-7 h-7 rounded-full text-ink-400 hover:bg-blush-100 hover:text-blush-500 transition grid place-items-center" aria-label="احذف">
            ...
            @if($cat->items->isNotEmpty())
                <div class="space-y-2 mb-3">
                    @foreach($cat->items as $item)
                        @include('menu.partials.item-row', ['item' => $item, 'L' => $L])
                    @endforeach
                </div>
            @else
                <p class="text-xs text-ink-400 text-center py-3 mb-2">مفيش {{ $L['item_label'] }} في {{ $cat->name }} لسه.</p>
            @endif

            {{-- Add item toggle (per category) --}}
            <button type="button" data-toggle-add-form
                    class="w-full text-center text-xs font-extrabold text-coral-600 hover:bg-coral-50 rounded-xl py-2 transition inline-flex items-center justify-center gap-1.5">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" class="w-3.5 h-3.5" data-toggle-icon-add>
                    <line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>
                </svg>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" class="w-3.5 h-3.5 hidden" data-toggle-icon-close>
                    <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
                </svg>
                <span data-toggle-label-open>أضف {{ $L['item_label'] }}</span>
                <span data-toggle-label-close class="hidden">إلغاء</span>
            </button>
            <div class="hidden mt-2" data-add-form-wrap>
                @include('menu.partials.add-item-form', ['business' => $business, 'categoryId' => $cat->id, 'L' => $L])
            </div>
        </div>
    @empty
    ...
    @if(isset($looseItems) && $looseItems->isNotEmpty())
        <div class="card-light p-4 mb-3 border-honey-500/20">
            <div class="flex items-center gap-2 mb-3 pb-3 border-b border-ink-950/8">
                ...
                <h3 class="text-sm font-bold text-ink-950">{{ $L['item_label'] }} بدون {{ $L['category_label'] }}</h3>
    ...
    <div class="card-light p-4">
        <button type="button" data-toggle-add-form
            ...
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" class="w-4 h-4" data-toggle-icon-add>
                <line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>
            </svg>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" class="w-4 h-4 hidden" data-toggle-icon-close>
                <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
            </svg>
            <span data-toggle-label-open>أضف {{ $L['item_label'] }} من غير {{ $L['category_label'] }}</span>
            <span data-toggle-label-close class="hidden">إلغاء</span>
        </button>
        <div class="hidden mt-3" data-add-form-wrap>
            @include('menu.partials.add-item-form', ['business' => $business, 'categoryId' => null, 'L' => $L])
        </div>
    </div>
</div>
...
<script>
    document.querySelectorAll('[data-toggle-add-form]').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const iconAdd   = btn.querySelector('[data-toggle-icon-add]');
            const iconClose = btn.querySelector('[data-toggle-icon-close]');
            const wasHidden = wrap.classList.contains('hidden');

            wrap.classList.toggle('hidden', !wasHidden);
            open.classList.toggle('hidden', wasHidden);
            close.classList.toggle('hidden', !wasHidden);
            iconAdd?.classList.toggle('hidden', wasHidden);
            iconClose?.classList.toggle('hidden', !wasHidden);

            if (wasHidden) wrap.querySelector('input[name="name"]')?.focus();
        });
    });
</script>
@endsection
