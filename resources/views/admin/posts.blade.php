...
<div class="flex items-center justify-between mb-4">
    ...
    <span class="text-ink-500 text-sm">{{ $posts->total() }}</span>
</div>
    ...
    @foreach($posts as $p)
        <div class="a-card p-4">
            <div class="flex items-start gap-3 mb-2">
                ...
                @if($p->status === 'removed')<span class="a-pill bg-blush-500 text-ink-950">محذوف</span>@endif
                <span class="ms-auto text-[11px] text-ink-400">{{ $p->created_at->diffForHumans() }}</span>
            </div>
            @if($p->title)<h3 class="font-bold mb-1">{{ $p->title }}</h3>@endif
            <p class="text-ink-500 text-sm leading-relaxed">{{ \Illuminate\Support\Str::limit($p->body, 200) }}</p>
